...
def del_all_syn_word(error_word):
    ...
    replacer = SynonymsReplacer(synonyms_file_path='./syn_txt/HIT-IRLab-Synonyms.txt',
                                cws_model_path='./ltp-models/3.4.0/ltp_data_v3.4.0/cws.model')
    syn_words = set()
    for w in words:
        print(w)
...
def test_sent(test_file, bug_file, name_bug_file, low_bug_file, s_id, e_id):
    test_list = load_trans_sent(test_file)
    sum = count_sentence(test_list)
    f1 = open(bug_file, mode="a")
        ...
        for i in range(0, len(sent_list[0])):
            ...
            if not result[0]:
                if len(result[1]) != 0:
                    ...
                    save_error_info(f1, errors)
        for i in range(0, len(sent_list) - 1):
            f1.write("add [" + str(i + 1) + "]" + "\n")
...
if __name__ == '__main__':
    ...
    name_bug_file = './bugs/' + file_name + "_" + mt_sys + '_name_bug.txt'
    low_bug_file = './bugs/' + file_name + "_" + mt_sys + '_low_bug.txt'
    start = time.perf_counter()
    read_stop_words()
    test_sent(translation_file, bug_file, name_bug_file, low_bug_file, s_id, e_id)
